    ...
    <el-table :data="tableData" border style="width: 100%">
      ...
      >
        <template slot-scope="scope">
          ...
          <el-tag type="primary" v-else-if="scope.row.category === '其他'">{{
            scope.row.category
          }}</el-tag>
          <el-tag v-else-if="scope.row.category === 'React'">{{
            scope.row.category
          }}</el-tag>
          <el-tag v-else type="primary">{{ scope.row.category }}</el-tag>
        </template>
      </el-table-column>
      ...
      <el-table-column prop="zuo" label="操作" width="300" align="center">
        <template slot-scope="scope" class="btn">
          ...
          <el-button type="danger" @click="onclick(scope.row._id)"
            >删除</el-button
          >
          <el-button type="success" @click="see(scope.row._id)">查看</el-button>
        </template>
      </el-table-column>
    </el-table>
  </div>
      ...
      axios
        ...
        .then((res) => {
          ...
          this.tableData.map((item) => {
            this.tableData.date = dayjs(localStorage.getItem("item")).format(
